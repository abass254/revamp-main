$(document).ready(function(){
    var dataListView = $(".data-list-view").DataTable({
        ...
        buttons: [
          {
            text: "<i class='feather icon-briefcase'></i><b> Add Job</b>",
            action: function() {
    ...
    $('.action-view').on("click", function(e){
        e.stopPropagation();
        window.location.href = "/job/" + jq(this).data('id');
    });
});
...
jq(function() {
    ...
    jq('li.payment-action').click(function(){
        jq("table.payment-details tbody tr").each(function(i, row) {
            if (i == 0){
                ...
                if (!jq(this).hasClass('hidden')){lowercase
    ...
    $('#modal-add-petty-cash').on('click', function(e) {
      ...
      $('#submitBtn').click(function(e) {
        ...
        var formData = $('#frmJob').serialize();
        var button = $(this);


        if ($('#jb_title').val() === '') {
          toastr.error('Job title should not be null', 'Creation Failed');
          return;
        }

        else if ($('#jb_description').val() === '') {
          toastr.error('Job description should not be null', 'Creation Failed');
          return;
        }

        else if ($('#jb_requirements').val() === '') {
          toastr.error('Job qualifications should not be null', 'Creation Failed');
          return;
        }
        ...
        $.ajax({
            ...
            data: formData,
            beforeSend: function () {
                toast = toastr.info('Please wait as the product is being confirmed...', 'Job Post', { closeButton: true, progressBar: true, timeOut: "10000" });
            ...
            success: function(response) {
                console.log(response);
                toastr.success(response.message, "Success"), { closeButton: true, progressBar: true, timeOut: "10000" };
            ...
            error: function (xhr, ajaxOptions, thrownError) {
                ...
                toastr.error(`Failed. Failed to post a job. ${xhr.responseJSON?.error ?? ""}`, 'User', { "closeButton": true, "progressBar": true });
            },
